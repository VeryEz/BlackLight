...
static int menu_callback_vehicles_sub ( int op, struct menu_item *item )
{
	if ( op == MENU_OP_SELECT && item->id != ID_NONE )
	{
		...
		struct vehicle_info *info = vehicle_info_get( item->id, VEHICLE_ALIVE | ((BlackLightFuncs->bVehicleFreeze) ? 0 : VEHICLE_EMPTY) );
		float				pos[3];
...
void menu_maybe_init ( void )
{
	...
	*menu_sampmisc, *menu_spoof_weapon, *menu_fake_kill, *menu_vehicles_instant, 
	*menu_specialaction;
