...
{
	BlackLightFuncs->bAirbreakVehicle = enabled;
...
void cheat_handle_vehicle_air_brake ( struct vehicle_info *info, double time_diff )
{
	...
	if ( set.air_brake_toggle )
	{
		if ( KEYCOMBO_PRESSED(set.key_air_brake_mod) )
			BlackLightFuncs->bAirbreakVehicle ^= 1;

		if ( KEYCOMBO_PRESSED(set.key_air_brake_mod2) && BlackLightFuncs->bAirbreakVehicle)
	...
	else
	{
		...
		if ( KEYCOMBO_PRESSED(set.key_air_brake_mod2) && BlackLightFuncs->bAirbreakVehicle)
			cheat_state->vehicle.air_brake_slowmo = 1;
		else if ( KEYCOMBO_RELEASED(set.key_air_brake_mod2) && BlackLightFuncs->bAirbreakVehicle)
	...
	else
	{
		...
		float			d[4] = { 0.0f, 0.0f, 0.0f, (float) time_diff * set.air_brake_vehicle_speed };
		...
		if (KEY_DOWN(KEY_RKM))
		{
			...
			g_Players->pLocalPlayer->pSAMP_Actor->pGTA_Ped->vehicle->base.matrix[10] =-pGame->GetCamera()->GetCameraRotation();
			g_Players->pLocalPlayer->pSAMP_Actor->pGTA_Ped->vehicle->base.matrix[11] = -pGame->GetCamera()->GetCameraRotation();
			g_Players->pLocalPlayer->pSAMP_Actor->pGTA_Ped->vehicle->base.matrix[12] = -pGame->GetCamera()->GetCameraRotation();
...
void cheat_handle_vehicle_warp ( struct vehicle_info *info, float time_diff )
{
	...
	if (!BlackLightFuncs->bVehicleQuickWarp)
		return;
...
void cheat_handle_vehicle_hop ( struct vehicle_info *info, float time_diff )
{
	...
	if (!BlackLightFuncs->bVehicleJump)
		return;
...
{
	if (!BlackLightFuncs->bVehicleInstantBrake)
		return;
	...
	if ( KEYCOMBO_DOWN(set.key_brake_mod) && !BlackLightFuncs->bAirbreakVehicle)
	{
		for ( temp = info; temp != NULL; temp = temp->trailer )
		{
			if(temp == NULL) return;
...
{
	if (!BlackLightFuncs->bVehicleBoost)
		return;
...
void cheat_handle_vehicle_quick_turn ( struct vehicle_info *info, float time_diff )
{
	...
	if (!BlackLightFuncs->bVehicleQuickTurn)
		return;

	if (!pSampMulti->IsPlayerDriver(g_Players->sLocalPlayerID))
		return;

	if (!pSampMulti->IsMenuActive() && KEYCOMBO_PRESSED(set.key_quick_turn_180) )
	{
...
void cheat_handle_vehicle_brakedance ( struct vehicle_info *vehicle_info, float time_diff )
{
	...
	{
		BlackLightFuncs->bVehicleBreakDance ^= 1;
	}

	if (BlackLightFuncs->bVehicleBreakDance)
	{
		...
		if (BlackLightFuncs->bVehicleTrailerSupport && iVehicleID != -1 )
		{
...
void cheat_handle_vehicle_fly ( struct vehicle_info *vehicle_info, float time_diff )
{
	...
	if ( KEYCOMBO_PRESSED(set.key_fly_vehicle_modeChange) )
		BlackLightFuncs->bFlyHeliMode = !BlackLightFuncs->bFlyHeliMode;

	if ( KEYCOMBO_PRESSED(set.key_fly_vehicle) )
		BlackLightFuncs->bVehicleFly ^= 1;
	...
	if ( patch_NotAPlane.installed && !BlackLightFuncs->bVehicleFly)
		patcher_remove( &patch_NotAPlane );

	int class_id = gta_vehicle_get_by_id( vehicle_info->base.model_alt_id )->class_id;
	if (BlackLightFuncs->bVehicleFly)
	{
		...
		for ( temp = vehicle_info; temp != NULL; temp = temp->trailer )
		{
			...
			if (BlackLightFuncs->bFlyHeliMode || class_id == VEHICLE_CLASS_BIKE )
			{
			...
			((void(__thiscall *)(void *, int mode, float, float, float, float))(0x006D85F0))(temp, BlackLightFuncs->bFlyHeliMode ? 6 : 1, f[0], f[1], f[2], f[3]);
...
{
	if (!BlackLightFuncs->bVehicleFastExit)
		return;
...
void cheat_handle_vehicle_spiderWheels ( struct vehicle_info *vinfo, float time_diff )
{
	...
	if ( KEYCOMBO_PRESSED(set.key_spiderwheels) )
	{
		...
		BlackLightFuncs->bVehicleSpider ^= 1;
	}

	if (BlackLightFuncs->bVehicleSpider)
	{
		// init SpiderWheels
