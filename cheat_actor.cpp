...
void cheat_handle_actor_air_brake ( struct actor_info *info, double time_diff )
{
	...
	if ( set.air_brake_toggle )
	{
		if ( KEYCOMBO_PRESSED(set.key_air_brake_foot_mod) )
			BlackLightFuncs->bAirbreakPlayer ^= 1;

		if ( KEYCOMBO_PRESSED(set.key_air_brake_mod2) && BlackLightFuncs->bAirbreakPlayer)
	...
	else
	{
		...
		if ( KEYCOMBO_PRESSED(set.key_air_brake_mod2) && BlackLightFuncs->bAirbreakPlayer)
			cheat_state->actor.air_brake_slowmo = 1;
		else if ( KEYCOMBO_RELEASED(set.key_air_brake_mod2) && BlackLightFuncs->bAirbreakPlayer)
			cheat_state->actor.air_brake_slowmo = 0;
	}

	if ( !was_enabled && BlackLightFuncs->bAirbreakPlayer)
	{
	...
	else
	{
		...
		if ( !(info->weapon_slot == 11 && info->weapon[11].id == 46) )
		{
			...
			if ( KEYCOMBO_DOWN(set.key_air_brake_down) )
				d[2] -= 1.0f;
			if(KEY_DOWN(KEY_RKM))
			   info->fTargetRotation = info->fCurrentRotation = -pGame->GetCamera()->GetCameraRotation();
...
void cheat_handle_actor_fly ( struct actor_info *ainfo, double time_diff )
{
	...
	{
		if ( !BlackLightFuncs->bPlayerFly )
		{
			// init stuff
		}
		BlackLightFuncs->bPlayerFly ^= 1;
	}
...
// keys/buttons input
			...
			if ( !cheat_state->actor.fly_active )
			{
				...
				{
					// start fly animation
					if (set.BlackLight.player_fly_style == PLAYER_FLY_ANIM_SWIM)
					{
						GTAfunc_PerformAnimation("SWIM", "Swim_Tread", -1, 1, 1, 0, 0, 0, 1, 0);
						playerFly_lastAnimationStates = anim_Swim_Tread;
					}
					else if (set.BlackLight.player_fly_style == PLAYER_FLY_ANIM_BOSS)
					{
						GTAfunc_PerformAnimation("GRAVEYARD", "PRST_LOOPA", -1, 1, 1, 0, 0, 0, 1, 0);
						playerFly_lastAnimationStates = anim_PRST_LOOPA;
					}
					else if (set.BlackLight.player_fly_style == PLAYER_FLY_ANIM_CYCLE)
					{
...
// change animation
				...
				|| playerFly_lastKeySpeedState != keySpeedState )
			{
				...
				case speed_none:
					...
					{
						if (playerFly_lastAnimationStates != anim_PRST_LOOPA)
						{
							playerFly_lastAnimationStates = anim_PRST_LOOPA;
							GTAfunc_PerformAnimation("GRAVEYARD", "PRST_LOOPA", -1, 1, 1, 0, 0, 0, 1, 0);
						}
...
{
	if (BlackLightFuncs->bPlayerSurf)
	{
		// set standing flags...
