...
void Log ( const char *fmt, ... )
{
	...
	{
		char	filename[512];
		snprintf( filename, sizeof(filename), "%s\\" M0D_FOLDER "%s", g_szWorkingDirectory, "BlackLight_CurrentSessionLog.log" );
	...
	if ( g_flLogAll == NULL )
	{
		g_flLogAll = fopen(std::string(M0D_FOLDER).append("BlackLight_AllLogs.log").c_str(), "a" );
		if ( g_flLogAll == NULL )
			return;
	}
...
void LogChatbox ( bool bLast, const char *fmt, ... )
{
	...
	{
		g_flLogChatbox = fopen(std::string(M0D_FOLDER).append("BlackLight_Chat.log").c_str(), "w" );
		if ( g_flLogChatbox == NULL )
			return;
	}
	...
	{
		g_flLogChatboxAll = fopen(std::string(M0D_FOLDER).append("BlackLight_AllChat.log").c_str(), "a" );
		if ( g_flLogChatboxAll == NULL )
			return;
	}
...
static int init ( void )
{
	...
	if ( g_hOrigDll == NULL )
	{
		...
		Log( "Compiled: %s %s CL:%d", __DATE__, __TIME__ ,COMPILE_VERSION );
