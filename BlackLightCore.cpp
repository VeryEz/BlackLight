				...
				if (vehicleType != CLIENTVEHICLE_BOAT || vehicleType != CLIENTVEHICLE_BIKE)
				{
					...
					if (not_tick_tak - ticktak > (DWORD)set.BlackLight.component_flood_delay)
					{
						...
						if (!state)
						{
							...
							for (int i = 0; i < 4; i++)
								pVehicle->GetDamageManager()->SetDoorStatus(eDoors(i), DT_DOOR_MISSING), pVehicle->GetDamageManager()->SetPanelStatus(ePanels(i), DT_DOOR_BASHED_AND_SWINGING_FREE);//визуал дверей
							state = true;
						}
						else
						{
							...
							GTAfunc_RepairVehicle(vinfo);
							for (int i = 0; i < 0; i++)
								pVehicle->GetDamageManager()->SetDoorStatus(eDoors(i), DT_DOOR_BASHED_AND_SWINGING_FREE), pVehicle->GetDamageManager()->SetPanelStatus(ePanels(i), DT_DOOR_BASHED_AND_SWINGING_FREE);//визуал дверей
							state = false;
	...
	if (bEnable)
	{
		...
		if (not_tick_tak - ticktak > (DWORD)set.BlackLight.wheel_status_pulse_delay)
		{
			...
			if (!state)
			{
				GTAfunc_RepairVehicle(vinfo);
				for (int i = 0; i < 4; i++)
					pVehicle->GetDamageManager()->SetWheelStatus(eWheels(i), DT_WHEEL_BURST);//визуал дверей
				state = true;
			}
			else
			{
				GTAfunc_RepairVehicle(vinfo);
				for (int i = 0; i < 0; i++)
					pVehicle->GetDamageManager()->SetWheelStatus(eWheels(i), DT_WHEEL_INTACT);//визуал дверей
				state = false;
